...
def get_character_dict(character_dict_path, character_type='ch', use_space_char=True):
    character_str = ''
...
## 根据推理结果解码识别结果
class ProcessPred(object):
    def __init__(self, dict_character):
            ...
            for idx in range(len(text_index[batch_idx])):
                ...
                if is_remove_duplicate:
                    if idx > 0 and text_index[batch_idx][idx - 1] == text_index[batch_idx][idx]:
                        continue
                char_list.append(self.dict_character[int(text_index[batch_idx][idx])])
...
def get_onet_sessions(det_onnx_file, rec_onnx_file, use_large=False, enable_profiling=False):
    options = onnxruntime.SessionOptions()
    options.enable_profiling = enable_profiling

    providers=['CPUExecutionProvider'] # 'CUDAExecutionProvider'
    onet_det_session = onnxruntime.InferenceSession(det_onnx_file, sess_options=options, providers=providers)

    if use_large:
        onet_rec_session = onnxruntime.InferenceSession(rec_onnx_file, sess_options=options, providers=providers)
    else:
        onet_rec_session = onnxruntime.InferenceSession(rec_onnx_file, sess_options=options, providers=providers)
    return onet_det_session, onet_rec_session


class OcrDetRec(object):

    def __init__(self, onet_det_session, onet_rec_session, dict_character):
